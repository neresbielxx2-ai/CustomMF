...
TONE_LABELS = {"profundo": "Profundo", "padrao": "Padrão", "suave": "Suave",
...
class SettingsPage:
    ...
    def build(self, parent):
        ...
        W.section_header(wrap, "⚙️", "Configurações",
                         "Tema, cores e dados do app. O fundo é escuro por padrão — "
                         "mas você manda em tudo por aqui.").pack(fill="x")
        ...
        tone_row.pack(fill="x", pady=8)
        cur_tone = config.get("theme.bg_tone", "padrao")
        for key, hexc in tones.items():
            sel = key == cur_tone
            cell = ctk.CTkFrame(tone_row, fg_color="transparent")
            cell.pack(side="left", padx=(0, 12))
            dot = ctk.CTkLabel(cell, text="", width=30, height=30, corner_radius=15,
                               fg_color=hexc, border_width=2,
                               border_color=t["accent"] if sel else t["border"])
            dot.pack(anchor="center")
            lbl = ctk.CTkLabel(cell, text=TONE_LABELS.get(key, key),
                               font=W.fnt(11, "bold" if sel else "normal"),
                               text_color=t["text"] if sel else t["sub"])
            lbl.pack()
            for wdg in (dot, lbl, cell):
                wdg.bind("<Button-1>", lambda _e, k=key: self._set_tone(k))
        ...
        W.ColorRow(abox, "Cor de destaque (menus, barras, gráficos)",
                   value=config.get("theme.accent", "#7C5CFF"),
                   on_change=self._set_accent).pack(fill="x", pady=(0, 12))
        W.ColorRow(abox, "Cor dos botões (voltar ao destaque usando o botão abaixo)",
                   value=config.get("theme.button_color") or config.get("theme.accent", "#7C5CFF"),
                   on_change=self._set_button).pack(fill="x")
        W.ghost_button(abox, "↩  Fazer os botões usarem a cor de destaque",
                       lambda: self._set_button(None)).pack(anchor="w", pady=(10, 0))
    ...
    def _preview(self, parent):
        t = self.t
        pv = ctk.CTkFrame(parent, corner_radius=14, fg_color=t["surface"],
