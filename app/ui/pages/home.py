...
from ...core import config, cursors, paths, theme
from .. import widgets as W

QUICK = [
    ("mouse", "🖱️", "Cursor do Mouse", "Pacotes macOS, Bibata, BreezeX ou uma imagem sua."),
...
class HomePage:
    ...
    def build(self, parent):
        ...
        wrap.pack(fill="both", expand=True, padx=30, pady=(24, 14))

        # ---------------- hero
        hero = W.Card(wrap, hover=False)
        hero.pack(fill="x", pady=(0, 16))
        hz = ctk.CTkFrame(hero, fg_color="transparent")
        hz.pack(fill="x", padx=22, pady=22)

        logo = None
        try:
            from PIL import Image
            logo = ctk.CTkImage(Image.open(paths.asset("logo.png")).convert("RGBA"), size=(72, 72))
        except Exception:
            logo = None
        if logo:
            ctk.CTkLabel(hz, image=logo, text="").pack(side="left", padx=(0, 18))

        htxt = ctk.CTkFrame(hz, fg_color="transparent")
        htxt.pack(side="left", fill="x", expand=True)
        h = datetime.datetime.now().hour
        sauda = "Bom dia" if 5 <= h < 12 else "Boa tarde" if 12 <= h < 18 else "Boa noite"
        ctk.CTkLabel(htxt, text=f"{sauda}! 👋", font=W.fnt(24, "bold"),
                     text_color=t["text"], anchor="w").pack(fill="x")
        ctk.CTkLabel(htxt, text="Pronto para deixar o seu Windows com a sua cara?",
                     font=W.fnt(12), text_color=t["sub"], anchor="w").pack(fill="x")

        hbtns = ctk.CTkFrame(hz, fg_color="transparent")
        hbtns.pack(side="right")
        W.primary_button(hbtns, "🖱️  Personalizar mouse",
                         lambda: self.mw.go("mouse")).pack(pady=(0, 6))
        W.ghost_button(hbtns, "✨  Ver efeitos", lambda: self.mw.go("click")).pack()

        # ---------------- status
        status = ctk.CTkFrame(wrap, fg_color=t["surface"], corner_radius=16,
                              border_width=1, border_color=t["border"])
        status.pack(fill="x", pady=(0, 16))
        box = ctk.CTkFrame(status, fg_color="transparent")
        box.pack(fill="x", padx=16, pady=12)
        ctk.CTkLabel(box, text="Status atual", font=W.fnt(11, "bold"),
                     text_color=t["sub"]).pack(anchor="w", pady=(0, 6))
        ...
        for txt, color in [
            (f"Cursor: {scheme}", t["accent"] if scheme not in ("", "Windows Padrão") else t["sub"]),
            (f"Efeitos: {'ligados' if fx_on else 'desligados'}", t["success"] if fx_on else t["sub"]),
            (f"Trilhas: {'ligadas' if tr_on else 'desligadas'}", t["success"] if tr_on else t["sub"]),
            (f"Fonte: {fonte}", t["accent"] if config.get("font.applied") else t["sub"]),
        ]:
            W.chip(row, txt, color).pack(side="left", padx=(0, 8))

        # ---------------- cartões de atalho
        grid = ctk.CTkFrame(wrap, fg_color="transparent")
        grid.pack(fill="x")
        for i, (key, icon, title, desc) in enumerate(QUICK):
            card = W.Card(grid, hover=True)
            ...
            top.pack(fill="x")
            W.icon_badge(top, icon, size=42, font_size=18).pack(side="left", padx=(0, 12))
            ctk.CTkLabel(top, text=title, font=W.fnt(15, "bold"),
                         text_color=t["text"]).pack(side="left")
            ctk.CTkLabel(inner, text=desc, font=W.fnt(12), text_color=t["sub"], anchor="w",
                         justify="left", wraplength=380).pack(fill="x", pady=(10, 12))
            W.primary_button(inner, "Abrir  →", lambda k=key: self.mw.go(k), width=104).pack(anchor="w")

        # ---------------- toggles rápidos
        quick = ctk.CTkFrame(wrap, fg_color=t["surface"], corner_radius=16,
                             border_width=1, border_color=t["border"])
        ...
        def mk_toggle(parent_, label, desc, key):
            fr = ctk.CTkFrame(parent_, fg_color=t["card"], corner_radius=14,
                              border_width=1, border_color=t["border"])
            fr.pack(side="left", expand=True, fill="x", padx=6)
            on = bool(config.get(key))
            sw_row = ctk.CTkFrame(fr, fg_color="transparent")
            sw_row.pack(fill="x", padx=14, pady=14)
